...
| 2026-08-14 | 移除旧版配置迁移提示 | `3602364` |
| 2026-08-14 | 助教视角收尾修复（配置/演示/README） | 本地修改，未发布 |

测试数量演变：362 → 363 → 368 → 375 → 378 → 384 → 396 → 420 → 425 → 427 → 443 → 469 → 478 → 480。
...
**任务**：按修订后的 PLAN 完成全部 32 个任务：核心类型与错误 → LLM 抽象层（trait + mock）→ 配置 → 工具系统（7 个工具）→ 护栏四层（最终顺序为静态规则/风险评估/沙箱/审批，含审计）→ 反馈闭环 → 记忆 → Agent 主循环 → 可观测性 → 子 Agent → 凭据 → TUI → CLI → Docker/CI → 机制演示。
...
- **两级配置**：全局 `~/.AuV/config.toml` + 项目 `./.AuV/config.toml`（cwd 为 home 跳过）；启动自动创建（已存在绝不改动，幂等）；`toml::Value` 字段级递归合并；`--config` 仍只读单文件；旧版项目根 `config.toml` 黄色迁移提示（该提示后于条目 29 移除）；损坏→中文错误退出
...
### 条目 29｜2026-08-14 移除旧版配置迁移提示

**任务**：用户要求移除旧版项目根 `config.toml` 的迁移提示（「那个配置迁移提示就不要了」）。

**实施**（commit `3602364`）：
- `LayeredPaths` 移除 `legacy` 字段；`resolve_config_paths` 不再解析旧版根 `config.toml` 路径
- `load_layered` 删除「3. 旧版配置迁移提示」分支（原「4. 合并结果反序列化」顺延为「3.」）
- 测试同步：`test_resolve_config_paths_basic` 移除 legacy 断言、`test_resolve_config_paths_cwd_is_home_skips_local` 改名并移除 legacy 断言；`test_load_layered_legacy_config_not_loaded_with_notice` 改名 `test_load_layered_legacy_config_silently_ignored`，断言旧版文件被静默忽略（不加载、无提示）
- 文档同步：SPEC §3.7、README 配置说明改为「不再加载（无提示）」

**验证**：全量 478 项测试通过、零编译警告。

---

### 条目 30｜2026-08-14 助教视角收尾修复（未发布）

**范围**：用户要求修复助教评测中除 `REFLECTION.md` 内容不足之外的其余问题，并明确暂不发布；本轮未改动 `docs/REFLECTION.md`，未 commit、push、打标签或创建 Release。

**关键修复**：
- 配置首次运行会在项目目录写入完整默认值，第二次运行因此用 `gpt-4o`/OpenAI 地址遮蔽全局 DeepSeek 配置。项目自动创建与 `auv init` 均改写仅含注释的稀疏覆盖模板，全局文件仍写完整默认值；新增首次/再次加载和 init 模板回归测试。
- Demo 2/4 原脚本 mock 不检查输入，后续响应无法证明反馈或子 agent 结果真的进入上下文。新增上下文检查型脚本 LLM，第二次调用必须看到对应 Tool 消息；四项演示按 1→4 串行输出，Demo 3 与生产管线统一为 L1 静态规则 → L2 风险评估 → L3 沙箱 → L4 审批。
- README 改用 HTTPS 克隆，补齐可直接运行的 GHCR `0.1.0` 与 v0.1.0 Release 下载/校验命令，说明 DeepSeek 等兼容 API 的配置前置、`doc/` 仅本机存在、文件记忆写侧缺口与明文 key 兼容边界；明文 `[llm] api_key` 使用时增加迁移警告。SPEC/PLAN 同步实际机制与凭据方案。

**验证**：`cargo check --all-targets --locked` 无警告；397 lib + 75 bin + 4 mechanism demo + 4 doctest = 480 项全部通过；`cargo build --release --locked` 成功。4 个 wiremock 测试在受限沙箱中因禁止绑定回环端口失败，使用本机权限重跑相同命令后通过。隔离临时项目连续两次使用真实全局 DeepSeek 配置执行 `auv run --no-tui`，分别返回 `AUV_QUICKSTART_OK` 与 `AUV_QUICKSTART_OK_2`，证明稀疏项目模板不再遮蔽全局配置。

---

## 统计汇总

- 提交总数：70（2026-07-08 至 2026-08-14，含 subagent 生产接入 13 个提交：步骤实现 11 + 文档同步 1 + 警告清除 1；配置迁移提示移除 1）
- 测试数量：362 → 480（+118）；当前测试构建零编译警告，Clippy 有约 300 条风格建议
